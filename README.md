...
- **音源ランキング（YouTube・/songs）**：TRENDとは別枠の専用ページです。YouTube Data API で「今伸びている音源関連動画」を複数の切り口（ダンス／新曲／バズった曲 等）で検索し、Geminiで曲名・アーティスト名・主な使用用途（踊ってみた／ネタ系／Vlog系／その他、動画タイトル・概要から読み取れる範囲でのAIの解釈）を抽出して週間TOP50（実際に特定できた件数まで）として保存します（`src/lib/song-sync.ts`）。曲名・アーティスト名を特定できなかった動画や、メドレー・まとめ系の動画はランキングに含めません。各曲にはMV／音源のYouTubeリンクを表示します。
