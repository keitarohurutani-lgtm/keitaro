...
ログイン・登録フォーム（`src/components/AuthForm.tsx`）は、Neonの無料DBが数日アクセスがないと自動スリープし復帰に数秒〜十数秒かかることがある点への対策として、送信から4秒経っても終わらない場合は「少し時間がかかっています」という案内を表示し、20秒経っても応答がなければタイムアウトとして扱い、ボタンを元の状態に戻してエラーメッセージ（再試行を促す文言）を表示します（`AbortController`使用）。対策前は、応答が返らないまま「処理中…」のボタンがいつまでも固まって見え、実際には壊れていないのに「ログインできない」と誤解される事例が発生していました。
...
`/analyze`はメインナビからは外していますが、機能・ページ・APIはそのまま残っています。ホーム画面の「STEP 4 分析」カードと「NEXT ACTION」の POST CHECK カードから引き続きアクセスできます。

## PLAYBOOK（ネタ集）について

`/playbook`は、スマホ1台で真似できる投稿の「型」を210個（7カテゴリー×30個）まとめた固定コンテンツです（`src/lib/playbook.ts`）。特定の実在トレンド動画には依存させず、時間が経っても内容が古くならない汎用フォーマットとして用意しています。カテゴリーを選ぶと、そのジャンルで今実際に伸びている実データ（Trend）もあわせて表示されます。`?category=SNS`のようにクエリパラメータでカテゴリーを指定して開くこともできます（ホーム画面のおすすめカードから使用）。

会員は各ネタの☆をタップしてお気に入り登録できます（`PlaybookFavorite`テーブル、`POST /api/playbook/[id]/favorite`でトグル）。お気に入りは会員ごとに保存され、「★ お気に入り」タブで絞り込んで見返せます。

## ホーム画面「あなたへのおすすめ」について

`src/lib/recommend.ts`の`getPlaybookRecommendations`が、会員ごとにPLAYBOOKのネタを3件提案します。ロジックは実データのみを根拠にしています：

- 保存済み企画（Idea）のトレンドカテゴリー、PLAYBOOKのお気に入り（お気に入りの方が明示的な意思表示として重み大）からカテゴリーごとの関心度を集計
- 関心度が高いカテゴリー順に1件ずつ提案し、そのカテゴリーには「『〇〇』をよくチェックしているあなたに」と実際の根拠を表示
- 活動実績がまだ少ない会員には、残りの枠を他カテゴリーから埋めて「今日のピックアップ」として提案（実績がないのに「あなた向け」と偽った理由はつけない）
- 同じ会員には同じ日なら同じ提案になるよう、日付ベースで決定的に選択（日替わりで内容は変わる）
