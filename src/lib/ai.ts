...
export async function summarizeYoutubeTrend(params: {
  ...
  const systemInstruction = [
    ...
    "専門用語や難しい言い回しを避け、中学生でも意味が分かるくらい平易な言葉・短い文で書いてください。",
  ].join("\n");
  ...
  return {
    ...
    properties: {
      ...
      overallComment: {
        type: "string",
        description:
          "動画全体を見て、次に自分が投稿するときに真似できそうな工夫を2〜3文で。" +
          "『良い動画ですね』のような感想ではなく、『自分ならこの部分でこうしてみる』が伝わる書き方にする。",
      },
      cuts: {
        type: "array",
        minItems: 3,
        ...
        items: {
          ...
          properties: {
            ...
            comment: {
              type: "string",
              description:
                "そのカットで実際に見える画角・カメラワーク・カット割り・編集（テロップ、BGM、トランジション等）を具体的に説明したうえで、" +
                "自分の次の投稿でそのまま真似できる一言（例：『自分も冒頭で〇〇を見せてみる』）を添える。",
            },
          },
...
export async function analyzeVideoFromUrl(videoUrl: string): Promise<VideoAnalysisResult> {
  ...
  const systemInstruction = [
    ...
    "具体的に説明してください。",
    "客観的な感想や評価だけで終わらせず、必ず『自分が次に投稿するならどう真似できるか』という",
    "具体的な行動に落とし込んで書いてください。",
    "動画に実際に映っていないことは書かないでください。断定的な『こうすべき』ではなく、",
    "『こういう工夫もあります』という提案口調で、中学生でも分かる易しい言葉で書いてください。",
  ].join("\n");
